...
int main(int argc, char * argv[]) {
	...
	PreProc_ pre_proc = getPreProc(diag->inputType(), pre_proc_type);
	...
	diag->initialize(cv_utils.getObj(0).corners, gen_flags);
	...
	int end_id = diag_end_id >= diag_start_id ? diag_end_id : input->n_frames - 1;
	if(end_id > start_id){
	...
	for(int frame_id = start_id; frame_id <= end_id; frame_id++){
		...
		diag->update(cv_utils.getGT(frame_id - diag_frame_gap));
