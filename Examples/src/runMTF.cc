	...
	*/
	...
	if(write_tracking_data){
		...
		if(tracking_data_fname.empty()){
			srand(time(nullptr));
			tracking_data_fname = cv_format("%s%d_%s_%s_%d", mtf_sm, rand(), mtf_am, mtf_ssm, 1 - hom_normalized_init);
		} else{
		...
		if(overwrite_gt){
			...
			for(int frame_id = 0; frame_id < init_frame_id; ++frame_id){
				mtf::utils::writeCorners(tracking_data_fid, cv_utils.getGT(frame_id), frame_id);
			}
		}
	...
	while(true) {
		...
		if(show_tracking_error || write_tracking_error || reinit_on_failure || show_ground_truth){
			gt_corners = resized_images ? cv_utils.getGT(input->getFrameID(), reinit_frame_id) / img_resize_factor :
				cv_utils.getGT(input->getFrameID(), reinit_frame_id);
		}
		cv::Mat tracker_corners = resized_images ? trackers[0]->getRegion() / img_resize_factor : trackers[0]->getRegion();
		...
		if(record_frames || show_cv_window) {
			...
			*/
			if(show_ground_truth){
				mtf::utils::drawRegion(input->getFrame(MUTABLE), gt_corners,
	...
	if(show_tracking_error){
		...
		if(input->n_frames > 0){
			printf(" / %d", input->n_frames - start_frame_id - 1);
		}
		printf("\n");
	}
	if(reinit_on_failure){
	...
	if(write_tracking_data){
		if(!reinit_on_failure && invalid_tracker_state && input->n_frames > 0){
			for(int frame_id = input->getFrameID(); frame_id < input->n_frames; ++frame_id){
