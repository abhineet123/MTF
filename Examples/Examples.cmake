...
if(WITH_PY)
	...
	if(PYTHONLIBS_FOUND AND PYTHON_NUMPY_FOUND)
		if(PYTHONLIBS_VERSION_STRING VERSION_LESS 3.0.0)
			...
			add_library(lk_cv MODULE Examples/cpp/lk_cv.cc)
			set_target_properties(lk_cv PROPERTIES PREFIX "")
			if(WIN32)
				set_target_properties(lk_cv PROPERTIES SUFFIX ".pyd")
			endif()
			target_compile_definitions(lk_cv PUBLIC ${MTF_DEFINITIONS})
			target_compile_options(lk_cv PUBLIC ${MTF_RUNTIME_FLAGS} ${MTF_COMPILETIME_FLAGS})
			target_include_directories(lk_cv PUBLIC ${MTF_INCLUDE_DIRS} ${MTF_EXT_INCLUDE_DIRS} ${PYTHON_INCLUDE_DIRS} ${PYTHON_NUMPY_INCLUDE_DIR})
			target_link_libraries(lk_cv mtf ${MTF_LIBS} ${PYTHON_LIBRARIES} ${PYTHON_LIBS})	
			install(TARGETS lk_cv LIBRARY DESTINATION ${MTF_PY_INSTALL_DIR} COMPONENT _lk_cv_)
			add_custom_target(_lk_cv_ DEPENDS lk_cv)
			set(EX_TARGET_NAMES ${EX_TARGET_NAMES} lk_cv)
		  else()
