...
if(WITH_PY)
	...
	if(PYTHONLIBS_FOUND AND PYTHON_NUMPY_FOUND)
		if(PYTHONLIBS_VERSION_STRING VERSION_LESS 3.0.0)
			...
			add_library(pyLK MODULE Examples/cpp/pyLK.cc)
			set_target_properties(pyLK PROPERTIES PREFIX "")
			if(WIN32)
				set_target_properties(pyLK PROPERTIES SUFFIX ".pyd")
			endif()
			target_compile_definitions(pyLK PUBLIC ${MTF_DEFINITIONS})
			target_compile_options(pyLK PUBLIC ${MTF_RUNTIME_FLAGS} ${MTF_COMPILETIME_FLAGS})
			target_include_directories(pyLK PUBLIC ${MTF_INCLUDE_DIRS} ${MTF_EXT_INCLUDE_DIRS} ${PYTHON_INCLUDE_DIRS} ${PYTHON_NUMPY_INCLUDE_DIR})
			target_link_libraries(pyLK mtf ${MTF_LIBS} ${PYTHON_LIBRARIES} ${PYTHON_LIBS})	
			install(TARGETS pyLK LIBRARY DESTINATION ${MTF_PY_INSTALL_DIR} COMPONENT _pyLK_)
			add_custom_target(_pyLK_ DEPENDS pyLK)
			set(EX_TARGET_NAMES ${EX_TARGET_NAMES} pyLK)

			add_library(pyWarp MODULE Examples/cpp/pyWarp.cc)
			set_target_properties(pyWarp PROPERTIES PREFIX "")
			if(WIN32)
				set_target_properties(pyWarp PROPERTIES SUFFIX ".pyd")
			endif()
			target_compile_definitions(pyWarp PUBLIC ${MTF_DEFINITIONS})
			target_compile_options(pyWarp PUBLIC ${MTF_RUNTIME_FLAGS} ${MTF_COMPILETIME_FLAGS})
			target_include_directories(pyWarp PUBLIC ${MTF_INCLUDE_DIRS} ${MTF_EXT_INCLUDE_DIRS} ${PYTHON_INCLUDE_DIRS} ${PYTHON_NUMPY_INCLUDE_DIR})
			target_link_libraries(pyWarp mtf ${MTF_LIBS} ${PYTHON_LIBRARIES} ${PYTHON_LIBS})	
			install(TARGETS pyWarp LIBRARY DESTINATION ${MTF_PY_INSTALL_DIR} COMPONENT _pyWarp_)
