...
int main(int argc, char * argv[]) {
	...
	if(!obj_utils.readObjectFromGT(seq_name, seq_path, input->getNFrames(),
	...
	patch_extractor_ssm.initialize(obj_utils.getGT(0));
	...
	for(int patch_id = 0; patch_id < n_extracted_patches; patch_id++){
		...
		std::string img_fname = cv::format("%s/%s/frame%05d.jpg", seq_path.c_str(), seq_name.c_str(), frame_id);
		...
		patch_extractor_ssm.setCorners(obj_utils.getGT(frame_id-1));
		patch_extractor_am.updatePixVals(patch_extractor_ssm.getPts());
		// **********DEBUG ******************
		//cout << "Using corners:\n";
		//cout << obj_utils.getGT(frame_id - 1]<<"\n";
		//cout << "SSM corners:\n";
