...
int main(int argc, char * argv[]) {
	...
	PreProc_ marker_pre_proc = mtf::getPreProc(marker_detector->inputType(), pre_proc_type);
	if(qr_input.empty()){
	...
	while(true) {
		...
		if(!marker_images_proc.empty()){
			...
			if(marker_found){
				...
				if(curr_qr_rect.width > qr_min_size && curr_qr_rect.height > qr_min_size &&
					...
					PreProc_ pre_proc = mtf::getPreProc(pre_procs, tracker->inputType(), pre_proc_type);
		...
		for(unsigned int tracker_id = 0; tracker_id < trackers.size(); ++tracker_id) {
			mtf::utils::drawRegion(input->getFrame(mtf::utils::MUTABLE), trackers[tracker_id]->getRegion());
		}
		if(qr_n_markers >= 3 && trackers.size() == qr_n_markers){
			...
			mtf::utils::drawRegion(input->getFrame(mtf::utils::MUTABLE), qr_region, cv::Scalar(255, 0, 0));
		}
		//	printf("n_trackers: %d\n", trackers.size());
		cv::imshow(main_window, input->getFrame(mtf::utils::MUTABLE));
