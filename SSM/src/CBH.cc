...
CBHParams::CBHParams(const SSMParams *ssm_params,
bool _normalized_init, double _grad_eps, bool _debug_mode):
SSMParams(ssm_params),
normalized_init(_normalized_init),
grad_eps(_grad_eps),
debug_mode(_debug_mode){}
CBHParams::CBHParams(const CBHParams *params) :
SSMParams(params),
normalized_init(CBH_NORMALIZED_BASIS),
grad_eps(CBH_GRAD_EPS),
debug_mode(CBH_DEBUG_MODE){
	if(params){
...
CBH::CBH(const ParamType *_params) :
...
void CBH::setCorners(const CornersT& corners){
...
void CBH::compositionalUpdate(const VectorXd& state_update){
...
void CBH::getWarpFromState(Matrix3d &warp_mat,
...
void CBH::getStateFromWarp(VectorXd &state_vec,
...
void CBH::invertState(VectorXd& inv_state, const VectorXd& state){
	//inv_state = -state;
...
void CBH::cmptInitPixJacobian(MatrixXd &dI_dp,
...
void CBH::cmptWarpedPixJacobian(MatrixXd &dI_dp,
	const PixGradT &dI_dw) {
	validate_ssm_jacobian(dI_dp, dI_dw);
...
void CBH::cmptPixJacobian(MatrixXd &dI_dp,
	const PixGradT &dI_dw){
	validate_ssm_jacobian(dI_dp, dI_dw);
...
void CBH::cmptApproxPixJacobian(MatrixXd &dI_dp,
	const PixGradT &dI_dw) {
	validate_ssm_jacobian(dI_dp, dI_dw);
...
void CBH::computeJacobian(MatrixXd &jacobian, Matrix24d &corners,
	Matrix3Xd &pts_hm){
	int state_id = 0;
...
void CBH::getPixGrad(Matrix2Xd &ssm_grad, int pt_id,
...
void CBH::estimateWarpFromCorners(VectorXd &state_update, const Matrix24d &in_corners,
...
void CBH::estimateWarpFromPts(VectorXd &state_update, vector<uchar> &mask,
