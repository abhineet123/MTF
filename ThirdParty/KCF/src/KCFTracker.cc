...
cv::Mat KCFTracker::convertFloatImg(cv::Mat &img){
	cv::Mat imgU;
	double minVal;
	double maxVal;
	cv::Point minLoc;
...
void KCFTracker::initialize(const cv::Mat& corners){
...
Point KCFTracker::ComputeMaxfl(Mat img)
{
	Mat imgU = img.clone();
	double minVal;
	double maxVal;
...
Point KCFTracker::updateCentroid(Point oldC, int w, int h, int imgw, int imgh)
{
...
void KCFTracker::preprocess(Mat img, Point centroid, int w, int h)
{
	double convertGray;
	int rows = img.rows;
	int cols = img.cols;
...
Rect KCFTracker::processFrame(cv::Mat img)
{
