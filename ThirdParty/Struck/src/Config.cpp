...
namespace struck{
	Config::Config(const std::string& path)
	{
		...
		while(getline(f, line))
		{
			...
			if(iss.fail() || name[0] == '#') continue;

			if(name == "strk_seed") iss >> seed;
			else if(name == "strk_quietMode") iss >> quietMode;
			else if(name == "strk_debugMode") iss >> debugMode;
			...
			else if(name == "strk_svmBudgetSize") iss >> svmBudgetSize;
			else if(name == "strk_feature")
			{
