...
CompressiveTracker::CompressiveTracker(const Paramtype *rct_params) :
...
void CompressiveTracker::initialize(const cv::Mat& init_corners){
...
void CompressiveTracker::update(){
	//cv::cvtColor(curr_img, curr_img_gs, CV_BGR2GRAY);
	processFrame(curr_img_gs, curr_rect);
	updateCVCorners();
}
void CompressiveTracker::updateCVCorners(){
