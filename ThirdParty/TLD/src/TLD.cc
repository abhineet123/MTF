...
#define TLD_TRACKER_ENABLED true
#define TLD_DETECTOR_ENABLED true
#define TLD_LEARNING_ENABLED true
#define TLD_ALTERNATING false
...
trackerEnabled(TLD_TRACKER_ENABLED),
detectorEnabled(TLD_DETECTOR_ENABLED),
learningEnabled(TLD_LEARNING_ENABLED),
alternating(TLD_ALTERNATING),
init_border_size(TLD_INIT_BORDER_SIZE){
	if(params){
...
namespace tld
{
	...
	void TLD::initialize(const cv::Mat& cv_corners){
		...
		//}
		printf("TLD :: best_fit_rect: x: %d y: %d width: %d height: %d\n",
	...
	void TLD::readFromFile(const char *path)
	{
		...
		for(int s = 0; s < numPositivePatches; s++)
		{
			...
			for(int i = 0; i < 15; i++)   //Do 15 times
			{
				...
				while(pch != NULL)
				{
					float val = static_cast<float>(atof(pch));
		...
		ec->numIndices = static_cast<int>(pow(2.0f, ec->numFeatures));
		ec->initPosteriors();
