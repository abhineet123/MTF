...
namespace nt{
	namespace gnn{
		GNN::GNN(DistTypePtr _dist_func, int _n_samples, int _n_dims,
			...
			printf("Using Graph based NN (NT) with:\n");
			printf("degree: %d\n", params.degree);
			printf("max_steps: %d\n", params.max_steps);
		...
		void GNN::buildGraph(const double *X, int k){
			nodes.resize(n_samples);
		...
		int GNN::searchGraph(const double *query, const double *X, int NNs, int K){
		...
		void GNN::pickKNNs(IndxDist *vis_nodes, int visited,
		...
		void GNN::knnSearch2(const double *Q, IndxDist *dists,
		...
		void GNN::knnSearch11(const double *Q, IndxDist *dists, const double *X,
		...
		void GNN::addNode(Node *node_i, int nn){
