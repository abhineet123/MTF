...
class Classifier {
 ...
 private:
  ...
  void WrapAnyLayer(std::vector<cv::Mat>* input_channels, shared_ptr< Blob<float> > layer);
...
void Classifier::WrapAnyLayer(std::vector<cv::Mat>* input_channels, shared_ptr<Blob<float> > layer) {
...
/*  const shared_ptr< Blob<float> > feature_blob= net_->blob_by_name("conv1");
  ...
  std::vector<Mat> fmap;
  WrapAnyLayer(&fmap,feature_blob ); 
