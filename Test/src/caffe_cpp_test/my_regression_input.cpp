...
/**Set Input Layer**/
void set_input_layer(shared_ptr<caffe::Net<float> > net, std::vector<cv::Mat> data, std::vector<cv::Mat> labels )
{

}

/** Training Function **/
int train(char *solver_file, char *weights_file, std::vector<cv::Mat> t_data, std::vector<cv::Mat> t_labels ) {
    ...
    for (int i=0; i<nepochs; i++)
    {
        for (int j=0; j<nbatches ; j++)
        {
            ...
            //set Input Blob
            set_input_layer(net_, t_data_b, t_labels_b);
